	...
	def readConfig(self):
		...
		for i in self.parser.sections():
			for j in self.parser.options(i):
				...
				elif j in ['login','passw','enc',  'downloaddir','customban','descr','pic','logsdir','incompletedir', 'autoreply', 'afterfinish', 'downloadregexp', 'afterfolder', 'default', 'chatfont', "npothercommand", "npplayer", "npformat"] or (i == "ui" and j not in ["roomlistcollapsed", "tabclosers",  'buddylistinchatrooms', "trayicon", "usernamehotspots", "exitdialog", "soundenabled", "transalpha",  "enabletrans", "speechenabled", "enablefilters", "width", "height"]):
