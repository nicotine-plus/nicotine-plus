...
should do when releasing a new version of Nicotine+.
 ...
 1. Update the translation template by running
 
    ```sh
    python3 data/scripts/update_translation_template.py
    ```

 2. Ensure that the source distribution (sdist) includes all necessary files to
 ...
 3. Add a new release note entry to NEWS.md. Release notes should contain a
 ...
 4. Ensure that Nicotine+ launches and works well on at least these operating
    systems:
    - Windows
    - macOS
    - Ubuntu 22.04 (oldest supported GTK 3, GTK 4 and Python 3 versions)
 ...
 6. On the [Packaging workflow](https://github.com/nicotine-plus/nicotine-plus/actions/workflows/packaging.yml)
    page, press the `Run workflow` button, enable `Create new release`, and run
    the workflow. This will build the release assets and prepare a release for
    deployment on GitHub and PyPI.

 7. Once the build process has finished, do a final review of the build
    artifacts if needed. If everything looks good, approve the deployment.
    If not, reject the deployment, make necessary changes, and repeat step 6.

 8. Verify that the new release appears on [GitHub](https://github.com/nicotine-plus/nicotine-plus/releases)
    and [PyPI](https://pypi.org/project/nicotine-plus/).

 9. Generate a stable PPA release for Ubuntu / Debian. First, ensure that the
