...
class MainWindow(Window):
    ...
    def shares_scanning(self, progress=None):
        ...
        if progress["sleeping"]:
            # TODO: Provide a better way to see where the scanner is stuck with
            # additional information such as time elapsed, file count and so on
            self.scan_progress_container.set_tooltip_text(progress["path"])
            return

        self.scan_progress_label.set_label(_("Scanned Folders: %s") % humanize(progress["num_folders"]))
