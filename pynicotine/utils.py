...
def human_duration(seconds, expand_seconds=False, expand_hours=False):
    """Returns a string for an approximatation of any amount of time in a
    sensible unit of measurement, for use where precision doesn't matter.

    expand_seconds gets seconds if under 2 minutes, instead of "1 minute".
    expand_hours gets hours if under 3 days, instead of "2 days" or "1 day".
    """

    seconds = int(seconds)

    if seconds < (120 if expand_seconds else 60):
    ...
    if hours < (72 if expand_hours else 24):
