...
/* eggtrayicon.c
 * Relicensed under LGPLv3 by daelstorm (2007)
...
static GdkFilterReturn egg_tray_icon_manager_filter(GdkXEvent * xevent, GdkEvent * event, gpointer user_data)
{
	...
	if (xev->xany.type == ClientMessage &&
		xev->xclient.message_type == icon->manager_atom && xev->xclient.data.l[1] == icon->selection_atom) {
...
#if HAVE_GTK_MULTIHEAD
		gdkwin = gdk_window_lookup_for_display(display, icon->manager_window);
#else
...
#if HAVE_GTK_MULTIHEAD
	gtk_plug_construct_for_display(GTK_PLUG(icon), gdk_screen_get_display(screen), 0);
#else
