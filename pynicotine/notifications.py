...
class Notifications:
    ...
    def show_notification(self, message, title=None) -> None:
        events.emit("show-notification", message, title=title)

    def show_chatroom_notification(self, room, message, title=None, high_priority=False) -> None:
        events.emit("show-chatroom-notification", room, message, title=title, high_priority=high_priority)

    def show_download_notification(self, message, title=None, high_priority=False) -> None:
        events.emit("show-download-notification", message, title=title, high_priority=high_priority)

    def show_private_chat_notification(self, username, message, title=None) -> None:
        events.emit("show-private-chat-notification", username, message, title=title)

    def show_search_notification(self, search_token, message, title=None) -> None:
        events.emit("show-search-notification", search_token, message, title=title)

    def show_upload_notification(self, message, title=None) -> None:
